...
use App\Http\Requests\Menu_detailRequest as StoreRequest;
use App\Http\Requests\Menu_detailRequest as UpdateRequest;
...
class Menu_detailCrudController extends CrudController
{
    public function setup()
    {
        ...
        |--------------------------------------------------------------------------
        */
        $this->crud->setModel('App\Models\Menu_detail');
        $this->crud->setRoute(config('backpack.base.route_prefix') . '/menu_detail');
        $this->crud->setEntityNameStrings('menu_detail', 'menu_details');
...
//        $this->crud->setFromDb();
        ...
        // $this->crud->addField($options, 'update/create/both');
         $this->crud->addFields([
             ['name' => 'id_menu', // The db column name
                 'label' => 'ID Menu', // Table column heading
                 'type' => 'text'],
    ...
    {
        $customer = DB::table("menu_detail")->get();

        $table_title = ['id','id_menu','id_food','quantity'];

        $oExcel =  Writer\WriterFactory::create(Type::XLSX); // for XLSX files
        $oExcel->openToBrowser("menu-detail.xlsx");
    ...
    public function ImportExcelAction(){
        if(isset($_FILES['excelFile']))
        {
            ...
            $title = ['id','id_menu','id_food','quantity'];
    ...
    public function import_CusDB($file_name,$title){
        ...
        foreach ($reader->getSheetIterator() as $sheet) {
            foreach ($sheet->getRowIterator() as $key=>$row) {
                ...
                elseif($key!=1 && implode($row)!=""){
                    if(gettype($row[2])!="integer" || gettype($row[3])!="integer"|| gettype($row[0])!="integer"){
                        $reader->close();
                        unlink($file_name);
                        return redirect()->back()->with("error","False Value Type");
                    }
                    $count_id = DB::table("menu_detail")->where("id",$row[0])->count();
                    if($count_id>0){
                        //Update
                        DB::table("menu_detail")->where("id",$row[0])->update([
                            "id_menu"=>$row[1],
                            "id_food"=>$row[2],
                            "quantity"=>$row[3]
                        ]);
